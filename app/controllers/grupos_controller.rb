class GruposController < ApplicationController
  ...
  def index
    if params[:query].present?
      @pagy, @grupos = pagy(Grupo.where("descricao like ?", "%#{params[:query]}%"))
    else
      @pagy, @grupos = pagy(Grupo.all)
    end

    if turbo_frame_request?
      render partial: "grupos", locals: { grupos: @grupos}
    else
      render :index
    end
  end
  ...
  private
    ...
    def grupo_params
      params.require(:grupo).permit(:nome_grupo, :descricao, :position)
    end
end
